...
//拉取第一类产品
$(function(){
	var categoryMarkup = '<div class="category_item"><a href="product?id=${id}"><img class="item_img" alt="${productName}" src="${img}"><span class="item_title">${productName}</span></a></div>';
	...
	ProductService.getProductByCatIdWithPage(3,3,1,{
		callback:function(data){
			//将获取到的数据绑定到页面上
			var markup = '<div class="feature_product"><a href="product?id=${id}"><img class="feature_img" src="${img}" alt="${productName}"/><div class="feature_text">{{html productFeature}}</div><div class="feature_title">${productName}</div></a></div>';
